...
Given(
    /The default OCR with classifier$/,
        ...
        lineStateAssociation = new Map(
            stateDestinationAssociation
                ...
                .map(([lineState, destination]) => {
                    ...
                    return [
                        LineState[
                            lineState as 'ERROR' | 'VALID' | 'UNREADABLE'
                        ],
                        filename,
                    ];
                })
...
Then(
    ...
    (destination: string, expectedContent: string) => {
        let fileContent: string;
        try {
            fileContent = fs
                .readFileSync(`features/test_files/out/${destination}`)
                .toString();
        } catch (e: any) {
